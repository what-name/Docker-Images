...
service mysql start
if [ ! -f /mysqlAlreadyDone.file ]; then
    mysql -u root < /srv/data/createDB.sql
    #### FIXME import the database itself now....
    mysql -u root -pNotThatGREATOFaPassword quotes < /srv/data/quotesDB.sql
    # Change root passwort FIXME why don't this work??????
    # Create a test file so that this won't run on every start of the container
    touch /mysqlAlreadyDone.file
fi
